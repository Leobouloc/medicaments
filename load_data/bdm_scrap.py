...
def get_tab_colon(_list, len_assert=True):
...
def get_tab(_list, len_assert=True):
    '''Renvoie un tableau selectionné par ses colonnes (à valeurs dans _list)'''
    def _contains(tab, _list):
        '''Renvoie true si le texte du tableau contient tous les strings de la liste'''
        return all([x in tab.text for x in _list])
...
def parse(file):
    ...
    corps = test = soup.findAll('td', {'colspan':'4'})[1].find('table')
    ...
    table_chemical_aux = get_tab(_list, len_assert = False)
    ...
    table_rembourement = str(get_tab(['Statut de remboursement']))
    
    #### START : table posologie
    table_posologie = str(get_tab(["Seuil d'alerte"]))
    
    #### START :  SMR
    smr = get_tab(['Service m', 'dical rendu'])
...
if __name__ == '__main__':
    file = os.path.join(path_BDM_scrap, '3400921600506.html')
    tab = parse(file)